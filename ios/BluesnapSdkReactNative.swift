...
class BluesnapSdkReactNative:  RCTEventEmitter {
    ...
    /*func generateAndSetBsToken(completion: @escaping (_ token: BSToken?, _ error: BSErrors?) -> Void) {
        ...
        DemoAPIHelper.createToken(shopperId: shopperId, completion: { resultToken, errors in
            ...
            do {
                try BlueSnapSDK.setBsToken(bsToken: self.thisbsToken)
                NSLog("generateAndSetBsToken, Got BS token= \(self.thisbsToken?.getTokenStr() ?? "")")
                DispatchQueue.main.async {
                    completion(resultToken, errors)
                }
                
            } catch {
                NSLog("generateAndSetBsToken, Unexpected error: \(error).")
            }
